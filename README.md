...
가상 테이블(CTE) 노드화: WITH절 임시 테이블이 물리 노드로 표시되는 현상 (필터링 로직 필요)

별칭(Alias) 파편화: LH., RD. 등 별칭에 따라 동일 컬럼이 중복 추출되는 이슈 (정규화 필요)

UNION 컨텍스트 소실: UNION 결합 시 각 블록별 필터 조건의 매핑 관계 모호 (블록 단위 파싱 필요)

사내 UDF 해석: 특화된 사용자 정의 함수의 함수명 식별 및 가중치 부여 기능 미흡
